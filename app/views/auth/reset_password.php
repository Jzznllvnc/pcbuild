<div class="relative w-full h-full flex items-center justify-center px-4 sm:px-6 lg:px-8 bg-cover min-h-screen bg-center pt-40 pb-72" style="background-image: url('/pcbuild/assets/images/CraftWisebg.png'); background-size: cover; background-position: center;">
    <div class="relative z-10 p-8 sm:p-10 bg-white shadow-2xl rounded-xl w-full max-w-md mx-auto">
        ...
        <?php if (!isset($error_code_mismatch) || $error_code_mismatch === false): // Only show form if no initial error from previous step ?>
            <form action="/pcbuild/public/reset-password" method="POST" class="space-y-6">
        ...
        <div class="mt-8 text-center">
            <p class="text-md text-gray-700">Go back to
                <a href="/pcbuild/public/login" class="font-bold text-[--color-dark-blue] hover:text-[#1a2d3a] hover:underline">Login</a>
            </p>
        </div>
    </div>
</div>